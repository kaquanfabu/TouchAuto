...
@interface UIWindow (TouchAuto)
@end

@implementation UIWindow (TouchAuto)
...
@interface UIApplication (TouchAuto)
@end

@implementation UIApplication (TouchAuto)
...
%ctor {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 2.0 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
        if (!gIsAppReady) {
            gIsAppReady = YES;
            [[UIApplication sharedApplication] setupTouchAuto];
        }
    });
}